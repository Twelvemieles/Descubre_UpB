using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

public class ControlPistas : MonoBehaviour {

    public Text pista;
    private string[] pistat = new string[20];

    private int npista;

    [SerializeField]
    private Transform[] pistap = new Transform[20];

    public GameObject pinPista;
    

    // Use this for initialization
    void Start() {

        
        pistat[0] = "Edificio donde te ofrecen atención médica.";
        pistat[1] = "La tienda de libros de la universidad.";
        pistat[2] = "De este edificio salen algunos de los mejores abogados del país.";
        pistat[3] = "Libros de todas las carreras y temáticas, si deseas leer alguno solo debes pedirlo.";
        pistat[4] = "Edificio donde muchos carros se encuentran, en varios pisos.";
        pistat[5] = "Si pierdes tu carnet deberás dirigirte a este bloque.";
        pistat[6] = "Una de las entradas de la universidad, la más cercana a la avenida Nutibara.";
        pistat[7] = "Uno de los edificios más nuevos, teniendo laboratorios de todo tipo.";
        pistat[8] = "El mayor espacio deportivo de la universidad.";
        pistat[9] = "El edificio de los especialistas en números.";
        pistat[10] = "El bloque más artístico de la universidad.";
        pistat[11] = "Edificio de posgrados. Aca podrás obtener tu maestria.";
        pistat[12] = "Centro de producción audiovisual.";
        pistat[13] = "Entrada de la universidad, por la avenida con el nombre de la universidad";
        pistat[14] = "Si deseas hacer parte de un club deportivo, debes pasar por aca.";
        pistat[15] = "Centro de las bases sociales humanisticas de la universidad.";
        pistat[16] = "Área donde se reúnen la mayor cantidad de estudiantes.";
        pistat[17] = "En este bloque encontraras la emisora radio Bolivariana.";
        pistat[18] = "Donde encontraras al padre rector.";
        pistat[19] = "Entrada de la universidad con 3 estatuas.";

        if(PlayerPrefs.HasKey("pista"))
        {
            PistaActualizar();
            Debug.Log("Entro Actualizar");
        }
        else
        {
            InicioPista();
            Debug.Log("Entro Inicio");
        }
        

    }
	
	// Update is called once per frame
	void Update () {
		
	}
   

    public void InicioPista()
    {
        int ran = Random.Range(1, 4);
        if(ran == 1)
        {
            npista = 19;
        }
        if(ran == 2)
        {
            npista = 6;
        }
        if(ran == 3)
        {
            npista = 13;
        }
        PlayerPrefs.SetInt("pista", npista);
        PistaActualizar();
    }

    public void PistaActualizar()
    {
        npista = PlayerPrefs.GetInt("pista");
        if (npista > 19)
        {
            npista = 0;
        }
        PlayerPrefs.SetInt("pista", npista);
        pista.text = pistat[npista];
        pinPista.transform.position = pistap[npista].position;
        
    }
    public void PistaCompleta()
    {
        if(npista >= 19)
        {
            npista = 0;
        }
        else
        {
            npista += 1;
        }
        PlayerPrefs.SetInt("pista", npista);
        PistaActualizar();
    }

    public void Deletekey()
    {
        PlayerPrefs.DeleteKey("pista");
    }


}
